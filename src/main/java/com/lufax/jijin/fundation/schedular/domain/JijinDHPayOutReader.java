 ...
 * 确认转出文件读取
 */
public class JijinDHPayOutReader {
    ...
    public JijinTradeConfirmDTO readLine(String lineContent, long lineNum, JijinSyncFileDTO file) {
        ...
        if (lineNum > 2) {
            ...
            dto.setFundCode(args[6]);
            dto.setConfirmDate(args[7]);
            dto.setAmount(new BigDecimal(args[8]));
            dto.setConfirmShare(new BigDecimal(args[9]));
            dto.setCompanyConfirmNo(args[10]);
