...
import com.lufax.jijin.fundation.repository.JijinTransferRepository;
import com.lufax.jijin.fundation.schedular.domain.JijinDHProfitReader;
import com.lufax.jijin.fundation.schedular.domain.JijinDHTransferReader;
...
@Service
public class ReadJijinDHTransferFileJob extends BaseBatchWithLimitJob<JijinSyncFileDTO, Long> {

    @Autowired
    private JijinSyncFileRepository jijinSyncFielRepository;
    @Autowired
    private JijinTransferRepository jijinTransferRepository;
    ...
    protected List<JijinSyncFileDTO> fetchList(int maxNum) {
        return jijinSyncFielRepository.findBusJijinSyncFileList(MapUtils.buildKeyValueMap("bizType", DHFileType.DH_TRANSFER.name(), "status", SyncFileStatus.READY.name(), "limit", maxNum));
    }
    ...
    public void process(JijinSyncFileDTO jijinSyncFileDTO) {
        try {
            ...
            recordDHTransferSync(jijinSyncFileDTO);
    ...
    private void recordDHTransferSync(JijinSyncFileDTO jijinSyncFileDTO) throws IOException {
    ...
    protected void dealFileWithBatchSize(File sourceFile, long startLine, long rownum, JijinSyncFileDTO syncFile) throws IOException {
        ...
        try {
            String s = null;
            List<JijinTransferDTO> dtos = new ArrayList<JijinTransferDTO>();
            JijinDHTransferReader contentreader = new JijinDHTransferReader();

            do {
                s = reader.readLine();
                if (StringUtils.isEmpty(s)) {// 文件读取完毕,插入最后一批数据
                    Logger.info(this, String.format("read dahua transfer file  - insert last batch records into DB endline:%s]", reader.getLineNumber() - 1));
                    if (dtos.size() > 0) {
                ...
                if (reader.getLineNumber() >= startLine && reader.getLineNumber() < startLine + rownum) {
                    // 解析行，转换成DTO
                    JijinTransferDTO dto = contentreader.readLine(s, reader.getLineNumber(), syncFile);
                    if (dto != null)
                        dtos.add(dto);
                }

                if (reader.getLineNumber() == startLine + rownum - 1) {// 已达到批次记录集，进行插数据库，清空缓存，并set下一个startLine
                    Logger.info(this, String.format("read dahua transfer file - insert into DB endline:%s]", reader.getLineNumber()));
    ...
    @Transactional
    public void batchInsertSyncAndUpdateSyncFile(List<JijinTransferDTO> dtos, long lineNum, JijinSyncFileDTO syncFile) {
    	jijinTransferRepository.batchInsertJijinTransfer(dtos);
