...
import com.lufax.jijin.fundation.schedular.domain.JijinDHPayInReader;
...
@Service
public class ReadJijinDHPurchaseFileJob extends BaseBatchWithLimitJob<JijinSyncFileDTO, Long> {
    ...
    protected List<JijinSyncFileDTO> fetchList(int maxNum) {
        return jijinSyncFielRepository.findBusJijinSyncFileList(MapUtils.buildKeyValueMap("bizType", DHFileType.DH_PAY_IN.name(), "status", SyncFileStatus.READY.name(), "limit", maxNum));
    }
    ...
    protected void dealFileWithBatchSize(File sourceFile, long startLine, long rownum, JijinSyncFileDTO syncFile) throws IOException {
        ...
        try {
            ...
            JijinDHPayInReader contentreader = new JijinDHPayInReader();

            do {
                s = reader.readLine();
                if (StringUtils.isEmpty(s)) {// 文件读取完毕,插入最后一批数据
                    Logger.info(this, String.format("read jijin dh purchase file  - insert last batch records into DB endline:%s]", reader.getLineNumber() - 1));
                    if (dtos.size() > 0) {
                ...
                if (reader.getLineNumber() == startLine + rownum - 1) {// 已达到批次记录集，进行插数据库，清空缓存，并set下一个startLine
                    Logger.info(this, String.format("read jijin dh purchase file - insert into DB endline:%s]", reader.getLineNumber()));
