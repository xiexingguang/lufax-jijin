...
public class DhEventService {
    ...
    public AccountAppCallerService getAccountAppCallerService() {
        return accountAppCallerService;
    }
    ...
    public void doPurchase(Long tradeRecordId) {
        Logger.info(this, String.format("dahua do purchase message be got ,tradeRecordId id [%s]", tradeRecordId));
        ...
        if (!jijinTradeRecordDTO.getStatus().equals(TradeRecordStatus.INIT.name())) {
            Logger.info(this, String.format("dahua tradeRecord is not init status,do nothing,tradeRecordId [%s]", tradeRecordId));
            return;
        }
        ...
        extension.setCdCard(accountDTO.getPayNo());
        ...
        String remark = "";
        Logger.info(this, String.format("dahua do purchase call jijin gw start... tradeRecordId is [%s]", tradeRecordId));
        ...
        Logger.info(this, String.format("dahua do purchase call jijin gw end...tradeRecordId is [%s], response is  [%s]", tradeRecordId, new Gson().toJson(gwResponseGson)));
        ...
        if (purchaseOrderResultGson.getErrorCode().equals("0000")) {
            ...
            if (jijinTradeRecordDTO.getStatus().equals(TradeRecordStatus.INIT.name()) && jijinTradeRecordDTO.getRetryTimes().compareTo(3L) < 0) {
                ...
                if (!"SUCCESS".equals(res.getStatus())) {
                    Logger.error(this, String.format("NEED_MANUAL_HANDLE,dahua do purchase failed,but call account do unfrozen failed ,tradeRecordId [%s]", jijinTradeRecordDTO.id()));
                    jijinTradeRecordRepository.updateBusJijinTradeRecord(MapUtils.buildKeyValueMap("id", tradeRecordId, "errorCode", purchaseOrderResultGson.getErrorCode(), "isControversial", "1", "status", TradeRecordStatus.FAIL.name(), "errorMsg", "改成调用大华基金公司返回0099或9999超过3次，解冻资金失败"));
                } else {
                    jijinTradeRecordRepository.updateBusJijinTradeRecord(MapUtils.buildKeyValueMap("id", tradeRecordId, "errorCode", purchaseOrderResultGson.getErrorCode(), "isControversial", "0", "status", TradeRecordStatus.FAIL.name(), "errorMsg", "改成调用大华基金公司返回0099或9999超过3次"));
                }
                ...
                if("LBO".equals(jijinTradeRecordDTO.getChannel())){
                	mqService.sendLBOTradeResultMsg(jijinTradeRecordDTO.getTrxId(), "fail","PURCHASE","");
                }else{
        	...
        	PaymentResult res=new PaymentResult();
        	 ...
        	 if (!"SUCCESS".equals(res.getStatus())) {
                Logger.error(this, String.format("NEED_MANUAL_HANDLE,dahua do purchase failed,but call account do unfrozen failed ,tradeRecordId [%s]", jijinTradeRecordDTO.id()));
                jijinTradeRecordRepository.updateBusJijinTradeRecord(MapUtils.buildKeyValueMap("id", tradeRecordId, "errorCode", purchaseOrderResultGson.getErrorCode(), "isControversial", "1", "status", TradeRecordStatus.FAIL.name(), "errorMsg", "调用大华基金公司下单失败,解冻失败"));
            } else {
                jijinTradeRecordRepository.updateBusJijinTradeRecord(MapUtils.buildKeyValueMap("id", tradeRecordId, "errorCode", purchaseOrderResultGson.getErrorCode(), "isControversial", "0", "status", TradeRecordStatus.FAIL.name(), "errorMsg", "调用大华基金公司下单失败"));
            }
    ...
    public void doWithdrawByPAF(Long tradeRecordId) {
        ...
        if (!jijinTradeRecordDTO.getStatus().equals(TradeRecordStatus.SUBMIT_SUCCESS.name())) {
            Logger.info(this, String.format("dahua current trade record status is not submit_success,event ignore tradeRecordId [%s]", tradeRecordId));
            return;
        }
        //幂等
        if (jijinTradeRecordDTO.getStatus().equals(TradeRecordStatus.WITHDRAW_SUCCESS.name())) {
            Logger.info(this, String.format("dahua current trade record has already do withdraw,event ignore tradeRecordId [%s]", tradeRecordId));
            return;
        }
        ...
        Logger.info(this, String.format("dahua do withdraw from paf request is [%s],tradeRecordId is [%s]", new Gson().toJson(pafPayRequestGson), tradeRecordId));
        GWResponseGson gwResponseGson = new GWResponseGson();

        try {
            gwResponseGson = pafGateWayRemoteService.pay(JsonHelper.toJson(pafPayRequestGson));
            Logger.info(this, String.format("dahua do withdraw from paf response is [%s],tradeRecordId is [%s]", new Gson().toJson(gwResponseGson), tradeRecordId));
        } catch (Exception e) {
            if (jijinTradeRecordDTO.getRetryTimes().compareTo(5L) < 0) {
                //小于5次的情况重试
                Logger.info(this, String.format("dahua call gwpaf-gw do withdraw fail,fail times less than 5,tradeRecordId is [%s]", tradeRecordId));
                jijinTradeRecordRepository.updateBusJijinTradeRecord(MapUtils.buildKeyValueMap("id", tradeRecordId, "retryTimes", jijinTradeRecordDTO.getRetryTimes() + 1));
                mqService.sendJijinOrderSuccessMsg(tradeRecordId);
                return;
            }
            Logger.info(this, String.format("dahua do paf withdraw retry last time still runtime exception fail, [%s],tradeRecordId is [%s]", new Gson().toJson(gwResponseGson), tradeRecordId));
        }
        ...
        if (gwResponseGson.isSuccess()) {
            ...
            if (pafPayResponseGson.getRespCode().equals("0000") || pafPayResponseGson.getRespCode().equals("0044")) {
                ...
                PaymentInfo info = new PaymentInfo(paymentOrderNo, frozenCode, "CLEAR_SUBTRACT", TransactionType.UNFROZEN_FUND.name(), remark,
                        new SubTractInfo(jijinTradeRecordDTO.getAppSheetNo(), jijinTradeRecordDTO.getReqAmount(), subTransactionType,
                        		remark, jijinInfoDTO.getInstId(), jijinInfoDTO.getProductCode(), jijinTradeRecordDTO.getTrxDate()), null);
                ...
                if (result) {
                    ...
                    jijinTradeRecordRepository.updateBusJijinTradeRecord(MapUtils.buildKeyValueMap("id", tradeRecordId, "errorCode", pafPayResponseGson.getRespCode(), "orderCompleteTime", pafPayResponseGson.getOrderCompleteTime(), "status", TradeRecordStatus.WITHDRAW_SUCCESS.name()));
                    mqService.sendJijinWithdrawSuccessMsg(tradeRecordId);

                } else {
                    //此处报警需要给先投资后代扣的tradeRecord做资金解冻并且扣减虚拟户
                    Logger.error(this, String.format("NEED_MANUAL_CONFIRM,dahua do withdraw call fund do unfrozen failed,tradeRecordId [%s]", tradeRecordId));
                }
                ...
                if (jijinTradeRecordDTO.getRetryTimes().compareTo(5L) < 0 || ("0043".equals(pafPayResponseGson.getRespCode()) && jijinTradeRecordDTO.getRetryTimes().compareTo(10L) < 0)) {
                    Logger.info(this, String.format("dahua do withdraw from paf fail times less than 5,tradeRecordId is [%s]", tradeRecordId));
                    if ("0043".equals(pafPayResponseGson.getRespCode()))
                        Logger.info(this, String.format("dahua do one more retry since paf response 0043handlepurc,tradeRecordId is [%s]", tradeRecordId));
                    jijinTradeRecordRepository.updateBusJijinTradeRecord(MapUtils.buildKeyValueMap("id", tradeRecordId, "retryTimes", jijinTradeRecordDTO.getRetryTimes() + 1));
                    mqService.sendJijinOrderSuccessMsg(tradeRecordId);
                } else {
                    if (jijinTradeRecordDTO.getFrozenType().equals("NORMAL")) {
                        ...
                        if ("SUCCESS".equals(res.getStatus())) {
                            jijinTradeRecordRepository.updateBusJijinTradeRecord(MapUtils.buildKeyValueMap("id", tradeRecordId, "errorCode", pafPayResponseGson.getRespCode(), "isControversial", "0", "status", TradeRecordStatus.FAIL.name(), "errorMsg", "调用平安付重试超过5次,平安付返回付款失败，交易置为失败", "errorCode", pafPayResponseGson.getRespCode()));
                            ...
                            Logger.error(this, String.format("NEED_MANUAL_CONFIRM,dahua do withdraw failed,call account do unfrozen failed,tradeRecordId [%s]", tradeRecordId));
                            jijinTradeRecordRepository.updateBusJijinTradeRecord(MapUtils.buildKeyValueMap("id", tradeRecordId, "errorCode", pafPayResponseGson.getRespCode(), "isControversial", "1", "status", TradeRecordStatus.FAIL.name(), "errorMsg", "重试超过5次,平安付返回付款失败，交易置为失败,但是虚拟户解冻失败，需人工解冻", "errorCode", pafPayResponseGson.getRespCode()));
                            jijinTradeLogRepository.insertBusJijinTradeLog(new JijinTradeLogDTO(jijinTradeRecordDTO.getUserId(), jijinInfoDTO.getFundCode(), jijinTradeRecordDTO.getId(), TradeRecordStatus.FAIL.name(), jijinTradeRecordDTO.getType(), jijinTradeRecordDTO.getReqAmount(), jijinTradeRecordDTO.getTrxTime(), jijinTradeRecordDTO.getTrxDate()));
                        }
                    } else {
                        ...
                        if (!"SUCCESS".equals(res.getStatus())) {
                            ...
                            jijinTradeRecordRepository.updateBusJijinTradeRecord(MapUtils.buildKeyValueMap("id", tradeRecordId, "errorCode", pafPayResponseGson.getRespCode(), "status", TradeRecordStatus.FAIL.name(), "isControversial", "0", "errorMsg", "调用平安付重试超过5次,平安付返回付款失败，交易置为失败", "errorCode", pafPayResponseGson.getRespCode()));
                            jijinTradeLogRepository.insertBusJijinTradeLog(new JijinTradeLogDTO(jijinTradeRecordDTO.getUserId(), jijinInfoDTO.getFundCode(), jijinTradeRecordDTO.getId(), TradeRecordStatus.FAIL.name(), jijinTradeRecordDTO.getType(), jijinTradeRecordDTO.getReqAmount(), jijinTradeRecordDTO.getTrxTime(), jijinTradeRecordDTO.getTrxDate()));
                            if("LBO".equals(jijinTradeRecordDTO.getChannel())){
                            	mqService.sendLBOTradeResultMsg(jijinTradeRecordDTO.getTrxId(), "fail","PURCHASE",pafPayResponseGson.getRespCode());
                            }else{
            ...
            if (jijinTradeRecordDTO.getRetryTimes().compareTo(5L) < 0) {
                ...
                Logger.info(this, String.format("dahua do withdraw from gwpaf-gw fail times large than 5, retCode [%s] tradeRecordId is [%s]", gwResponseGson.getRetCode(), tradeRecordId));
    ...
    public void buyApplyNotify(Long tradeRecordId) {
        JijinTradeRecordDTO jijinTradeRecordDTO = jijinTradeRecordRepository.getRecordById(tradeRecordId);
        if (!jijinTradeRecordDTO.getStatus().equals(TradeRecordStatus.WITHDRAW_SUCCESS.name())) {
            Logger.info(this, String.format("dahua tradeRecord status is not withdraw_success,do nothing,tradeRecordId [%s]", tradeRecordId));
            return;
        }
        ...
        if ("0000".equals(gwBuyApplyNotifyResponseGson.getErrorCode())) {
            updateTradeStatusAndUserBalance(jijinTradeRecordDTO, gwBuyApplyNotifyResponseGson, notifyAppNo);
        } else {
    ...
    @Transactional
    public void updateTradeStatusAndUserBalance(JijinTradeRecordDTO jijinTradeRecordDTO, GWBuyApplyNotifyResponseGson gwBuyApplyNotifyResponseGson, String notifyAppNo) {

        jijinTradeRecordRepository.updateBusJijinTradeRecord(MapUtils.buildKeyValueMap("id", jijinTradeRecordDTO.getId(), "errorCode", gwBuyApplyNotifyResponseGson.getErrorCode(), "notifyAppNo", notifyAppNo, "status", TradeRecordStatus.SUCCESS.name()));
