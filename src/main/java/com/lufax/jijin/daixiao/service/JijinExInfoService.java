...
import com.lufax.jijin.daixiao.schedular.domain.JijinExInfoReader;
...
@Service
public class JijinExInfoService {
    ...
    public void dealFileWithBatchSize(File sourceFile, long startLine, long rownum, JijinSyncFileDTO syncFile) throws IOException {
        ...
        try {
            String s = null;
            List<JijinExInfoDTO> dtos = new ArrayList<JijinExInfoDTO>();
            JijinExInfoReader contentreader = new JijinExInfoReader();
            ...
            do {
                ...
                if (reader.getLineNumber() >= startLine && reader.getLineNumber() < startLine + rownum) {
                    // 解析行，转换成DTO
                    JijinExInfoDTO dto = contentreader.readLine(s, reader.getLineNumber(), syncFile);
                    if (dto != null)
                        dtos.add(dto);
                }
    ...
    @Transactional
    public void batchInsertSyncAndUpdateSyncFile(List<JijinExInfoDTO> dtos, long lineNum, JijinSyncFileDTO syncFile) {
        jijinExInfoRepository.batchInsertDTOs(dtos);
        jijinSyncFileRepository.updateBusJijinSyncFile(MapUtils.buildKeyValueMap("id", syncFile.getId(), "currentLine", lineNum));
    }

    @Transactional
    public void dispatchJijinExInfo(JijinExInfoDTO jijinExInfoDTO) {
        jijinExInfoDTO = jijinExInfoRepository.getJijinExInfoById(jijinExInfoDTO.getId());
        if (!jijinExInfoDTO.getStatus().equals(RecordStatus.NEW.name())) {
            return;
        }

        JijinInfoDTO jijinInfoDTO = jijinInfoRepository.findJijinInfoByFundCode(jijinExInfoDTO.getFundCode());
        if (jijinInfoDTO == null) {
            Logger.info(this, String.format("can not find jijinInfo with fundCode [%s],do nothing", jijinExInfoDTO.getFundCode()));
            jijinExInfoRepository.updateJijinExInfo(MapUtils.buildKeyValueMap("id", jijinExInfoDTO.getId(), "status", RecordStatus.NO_USED.name()));
            return;
        } else {
            JijinUpdateGson jijinUpdateGson = new JijinUpdateGson();
            jijinUpdateGson.setCode(jijinInfoDTO.getProductCode());
            jijinUpdateGson.setTrustee(jijinExInfoDTO.getCustodianBank());//托管人
            jijinUpdateGson.setFundBrand(jijinExInfoDTO.getCompanyCode());//管理人对应基金品牌
            jijinUpdateGson.setEstablishedDate(DateUtils.formatDateFromStringToString(jijinExInfoDTO.getSetupDate()) + " 00:00:00");//成立日期
            jijinUpdateGson.setDisplayName(jijinExInfoDTO.getName());//名称,传简称@槐远0820

            jijinUpdateGson.setFundManagerName(jijinExInfoDTO.getManagementComp());//管理人
            ...
            if (createJijinInfoResultGson.getRetCode().equals("000")) {
                jijinExInfoRepository.updateJijinExInfo(MapUtils.buildKeyValueMap("id", jijinExInfoDTO.getId(), "status", RecordStatus.DISPACHED.name()));
            } else {
                jijinExInfoRepository.updateJijinExInfo(MapUtils.buildKeyValueMap("id", jijinExInfoDTO.getId(), "status", RecordStatus.FAILED.name(), "errorMsg", createJijinInfoResultGson.getRetMessage()));
            }
        }
    }
}
