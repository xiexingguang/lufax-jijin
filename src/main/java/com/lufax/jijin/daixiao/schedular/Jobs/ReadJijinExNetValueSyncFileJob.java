...
@Service
public class ReadJijinExNetValueSyncFileJob extends BaseBatchWithLimitJob<JijinSyncFileDTO, Long> {
    ...
    protected List<JijinSyncFileDTO> fetchList(int batchAmount) {
        return jijinSyncFielRepository.findBusJijinSyncFileList(MapUtils.buildKeyValueMap("bizType", DaiXiaoFileTypeEnum.净值.getTypeName(), "status", SyncFileStatus.READY.name(), "limit", batchAmount));
    }
