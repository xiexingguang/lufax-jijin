 ...
 */
@Service
public class ReadJijinExBuyLimitSyncFileJob extends BaseBatchWithLimitJob<JijinSyncFileDTO, Long> {
    @Autowired
    ...
    protected List<JijinSyncFileDTO> fetchList(int batchAmount) {
        return jijinSyncFielRepository.findBusJijinSyncFileList(MapUtils.buildKeyValueMap("bizType", DaiXiaoFileTypeEnum.认申购起点限额.getTypeName(), "status", SyncFileStatus.READY.name(), "limit", batchAmount));
    }
