 ...
 */
@Service
public class ReadJijinExIncomeModeSyncFileJob extends BaseBatchWithLimitJob<JijinSyncFileDTO, Long> {
    @Autowired
    ...
    protected List<JijinSyncFileDTO> fetchList(int batchAmount) {
        return jijinSyncFielRepository.findBusJijinSyncFileList(MapUtils.buildKeyValueMap("bizType", DaiXiaoFileTypeEnum.默认分红方式.getTypeName(), "status", SyncFileStatus.READY.name(), "limit", batchAmount));
    }
