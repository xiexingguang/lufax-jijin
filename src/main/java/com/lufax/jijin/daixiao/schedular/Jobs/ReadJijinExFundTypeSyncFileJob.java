...
@Service
public class ReadJijinExFundTypeSyncFileJob extends BaseBatchWithLimitJob<JijinSyncFileDTO, Long> {
    ...
    protected List<JijinSyncFileDTO> fetchList(int batchAmount) {
        return jijinSyncFielRepository.findBusJijinSyncFileList(MapUtils.buildKeyValueMap("bizType", DaiXiaoFileTypeEnum.基金分类.getTypeName(), "status", SyncFileStatus.READY.name(), "limit", batchAmount));
    }
