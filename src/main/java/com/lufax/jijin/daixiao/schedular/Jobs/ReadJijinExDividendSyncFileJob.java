...
import com.lufax.jijin.daixiao.service.JijinExDividendService;
...
@Service
public class ReadJijinExDividendSyncFileJob extends BaseBatchWithLimitJob<JijinSyncFileDTO, Long> {
    ...
    protected List<JijinSyncFileDTO> fetchList(int batchAmount) {
        return jijinSyncFielRepository.findBusJijinSyncFileList(MapUtils.buildKeyValueMap("bizType", DaiXiaoFileTypeEnum.历史分红信息.getTypeName(), "status", SyncFileStatus.READY.name(), "limit", batchAmount));
    }
