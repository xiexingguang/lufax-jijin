...
@Service
public class ReadJijinExScopeSyncFileJob extends BaseBatchWithLimitJob<JijinSyncFileDTO, Long> {
    ...
    protected List<JijinSyncFileDTO> fetchList(int batchAmount) {
        return jijinSyncFielRepository.findBusJijinSyncFileList(MapUtils.buildKeyValueMap("bizType", DaiXiaoFileTypeEnum.规模及规模变动.getTypeName(), "status", SyncFileStatus.READY.name(), "limit", batchAmount));
    }
