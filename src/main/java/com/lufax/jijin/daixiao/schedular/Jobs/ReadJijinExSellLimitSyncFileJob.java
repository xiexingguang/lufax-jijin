 ...
 */
@Service
public class ReadJijinExSellLimitSyncFileJob extends BaseBatchWithLimitJob<JijinSyncFileDTO, Long> {
    @Autowired
    ...
    protected List<JijinSyncFileDTO> fetchList(int batchAmount) {
        return jijinSyncFielRepository.findBusJijinSyncFileList(MapUtils.buildKeyValueMap("bizType", DaiXiaoFileTypeEnum.赎回转换起点.getTypeName(), "status", SyncFileStatus.READY.name(), "limit", batchAmount));
    }
