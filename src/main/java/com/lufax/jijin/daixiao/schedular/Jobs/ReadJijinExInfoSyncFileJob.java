...
@Service
public class ReadJijinExInfoSyncFileJob extends BaseBatchWithLimitJob<JijinSyncFileDTO, Long> {
    ...
    protected List<JijinSyncFileDTO> fetchList(int batchAmount) {
        return jijinSyncFielRepository.findBusJijinSyncFileList(MapUtils.buildKeyValueMap("bizType", DaiXiaoFileTypeEnum.基金基本要素.getTypeName(), "status", SyncFileStatus.READY.name(), "limit", batchAmount));
    }
