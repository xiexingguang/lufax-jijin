...
@Service
public class ReadJijinExMfPerformSyncFileJob extends BaseBatchWithLimitJob<JijinSyncFileDTO, Long> {
    ...
    protected List<JijinSyncFileDTO> fetchList(int batchAmount) {
        return jijinSyncFielRepository.findBusJijinSyncFileList(MapUtils.buildKeyValueMap("bizType", DaiXiaoFileTypeEnum.基金净值增长率.getTypeName(), "status", SyncFileStatus.READY.name(), "limit", batchAmount));
    }
