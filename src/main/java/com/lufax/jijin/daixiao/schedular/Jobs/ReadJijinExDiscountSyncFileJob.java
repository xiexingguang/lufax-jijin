 ...
 */
@Service
public class ReadJijinExDiscountSyncFileJob extends BaseBatchWithLimitJob<JijinSyncFileDTO, Long> {
    @Autowired
    ...
    protected List<JijinSyncFileDTO> fetchList(int batchAmount) {
        return jijinSyncFielRepository.findBusJijinSyncFileList(MapUtils.buildKeyValueMap("bizType", DaiXiaoFileTypeEnum.优惠费率.getTypeName(), "status", SyncFileStatus.READY.name(), "limit", batchAmount));
    }
