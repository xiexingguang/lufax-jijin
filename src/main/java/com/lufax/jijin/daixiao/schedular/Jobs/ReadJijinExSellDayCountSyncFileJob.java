 ...
 */
@Service
public class ReadJijinExSellDayCountSyncFileJob extends BaseBatchWithLimitJob<JijinSyncFileDTO, Long> {
    @Autowired
    ...
    protected List<JijinSyncFileDTO> fetchList(int batchAmount) {
        return jijinSyncFielRepository.findBusJijinSyncFileList(MapUtils.buildKeyValueMap("bizType", DaiXiaoFileTypeEnum.赎回到账日期.getTypeName(), "status", SyncFileStatus.READY.name(), "limit", batchAmount));
    }
