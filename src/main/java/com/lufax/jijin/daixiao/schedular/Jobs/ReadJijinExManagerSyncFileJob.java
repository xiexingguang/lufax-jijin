...
@Service
public class ReadJijinExManagerSyncFileJob extends BaseBatchWithLimitJob<JijinSyncFileDTO, Long> {
    ...
    protected List<JijinSyncFileDTO> fetchList(int batchAmount) {
        return jijinSyncFielRepository.findBusJijinSyncFileList(MapUtils.buildKeyValueMap("bizType", DaiXiaoFileTypeEnum.基金经理.getTypeName(), "status", SyncFileStatus.READY.name(), "limit", batchAmount));
    }
