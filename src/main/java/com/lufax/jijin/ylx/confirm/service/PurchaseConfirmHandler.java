...
@Service
public class PurchaseConfirmHandler {
	...
	public boolean handleBuyConfirmData(YLXBuyRequestDetailDTO ylxBuyRequestDTO, Map<Long, BuyConfirmDetailDTO> buyConfirmMap){
        if (buyConfirmMap.containsKey(ylxBuyRequestDTO.getId())) {
            ...
            if (ylxBuyRequestDTO.getAmount().compareTo(ylxBuyConfirmDTO.getAmount()) == 0) {
            	...
            	ylxBuyRequestDTO.setConfirmUnitPrice(ylxBuyConfirmDTO.getConfirmUnitPrice());
            	ylxBuyRequestDTO.setPurchaseFee(ylxBuyConfirmDTO.getPurchaseFee());
            	ylxBuyRequestDTO.setFundShare(ylxBuyConfirmDTO.getConfirmFundShare());
                ...
                if (fundAccountDTO == null) {
                    ...
                    mqService.sendYLXBuyConfirmMsg(String.format("%s-%s", ylxBuyRequestDTO.getBatchId(), ylxBuyRequestDTO.id()), ylxBuyRequestDTO.getInternalTrxId(), YLXTransactionHistoryType.PURCHASE.name(), false);
                    return false;
                }
                ...
                if (ylxBuyRequestDTO.getStatus().equals(YLXBuyRequestStatus.SUCCESS.name())) {
                    ...
                    ylxBuyRequestDetailRepository.update("updateBuyRequestById",MapUtils.buildKeyValueMap(
                    				"id", ylxBuyRequestDTO.getId(), 
                    				"status", ylxBuyRequestDTO.getStatus(),
                    				"purchaseFee",ylxBuyRequestDTO.getPurchaseFee(),
                    				"confirmUnitPrice",ylxBuyRequestDTO.getConfirmUnitPrice(),
                    				"fundShare",ylxBuyRequestDTO.getFundShare()
                    				));
                    return true;
                } else {
                    ylxBuyRequestDetailRepository.update("updateBuyRequestStatusById", MapUtils.buildKeyValueMap("id", ylxBuyRequestDTO.getId(), "status", ylxBuyRequestDTO.getStatus()));
    ...
    private void updateFundBalance(YLXBuyRequestDetailDTO ylxBuyRequestDetailDTO,FundAccountDTO fundAccountDTO,BuyConfirmDetailDTO ylxBuyConfirmDTO){
    	...
    	ylxTransactionHistoryDTO.setRemark("申购交易交易记录");
    	ylxTransactionHistoryDTO.setTransactionType(YLXTransactionHistoryType.PURCHASE.name());
    	ylxTransactionHistoryDTO.setTransactionAmount(ylxBuyRequestDetailDTO.getAmount());
        ylxTransactionHistoryDTO.setFundShare(ylxBuyConfirmDTO.getConfirmFundShare());
