...
@Component
public class YLXPurchaseRequestFileCreationJob extends BaseJob<YLXBatchDTO,Long> {
    @Autowired
    ...
    protected List<YLXBatchDTO> fetchList(int batchAmount) {
        return ylxPurchaseRequestService.getBatchRequestRecords(YLXBatchType.YLX_SLP_PURCHASE_REQUEST, YLXBatchStatus2.REQUEST_DATA_PREPARED, BatchRunStatus.IDLE);
    }
    ...
    protected void processList(List<YLXBatchDTO> list) {
        for(YLXBatchDTO batchDTO : list){
            try {
                ...
                ylxPurchaseRequestService.createFiles(batchDTO);
            }catch (Exception e){
                Logger.error(this, String.format("YLXPurchaseRequestFileCreationJob failed,batchId:%s", batchDTO.getId()),e);
            }finally {
