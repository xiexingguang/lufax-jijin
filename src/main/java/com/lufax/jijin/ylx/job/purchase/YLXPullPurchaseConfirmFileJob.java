...
@Component
public class YLXPullPurchaseConfirmFileJob extends BaseJob<YLXBatchDTO,Long> {
    @Autowired
    ...
    protected List<YLXBatchDTO> fetchList(int batchAmount) {
        return baseConfirmServie.getAfterTriggerDateBatchRecords(YLXBatchType.YLX_SLP_PURCHASE_CONFIRM, asList(YLXBatchStatus2.CONFIRM_RECEIVE), BatchRunStatus.IDLE);
    }
    ...
    protected void processList(List<YLXBatchDTO> list) {
        for(YLXBatchDTO batchDTO : list){
            try {
                ...
                Logger.error(this, String.format("YLXPullPurchaseConfirmFileJob failed,batchId:%s", batchDTO.getId()),e);
            }finally {
