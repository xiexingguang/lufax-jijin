...
@Component
public class YLXBuyRequestFileCreationJob extends BaseJob<YLXBatchDTO,Long> {
    @Autowired
    ...
    protected List<YLXBatchDTO> fetchList(int batchAmount) {
        return ylxBuyRequestService.getBatchRequestRecords(YLXBatchType.YLX_SLP_BUY_REQUEST, YLXBatchStatus2.REQUEST_DATA_PREPARED, BatchRunStatus.IDLE);
    }
    ...
    protected void processList(List<YLXBatchDTO> list) {
        for(YLXBatchDTO batchDTO : list){
            try {
                ...
                ylxBuyRequestService.createFiles(batchDTO);
            }catch (Exception e){
