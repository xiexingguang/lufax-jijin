...
@Component
public class YLXPullRedeemConfirmFileJob extends BaseJob<YLXBatchDTO,Long> {
    @Autowired
    ...
    protected List<YLXBatchDTO> fetchList(int batchAmount) {
        return baseConfirmServie.getAfterTriggerDateBatchRecords(YLXBatchType.YLX_SLP_REDEEM_CONFIRM, asList(YLXBatchStatus2.CONFIRM_RECEIVE), BatchRunStatus.IDLE);
    }
    ...
    protected void processList(List<YLXBatchDTO> list) {
        for(YLXBatchDTO batchDTO : list){
            try {
                ...
                Logger.error(this, String.format("YLXPullRedeemConfirmFileJob failed,batchId:%s", batchDTO.getId()),e);
            }finally {
