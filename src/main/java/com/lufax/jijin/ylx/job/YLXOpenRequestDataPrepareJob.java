...
@Component
public class YLXOpenRequestDataPrepareJob extends BaseJob<YLXBatchDTO,Long> {
    @Autowired
    ...
    protected List<YLXBatchDTO> fetchList(int batchAmount) {
        return ylxOpenRequestService.getBatchRequestRecords(YLXBatchType.YLX_SLP_OPEN_REQUEST, YLXBatchStatus2.REQUEST_INIT, BatchRunStatus.IDLE);
    }
    ...
    protected void processList(List<YLXBatchDTO> list) {
        for(YLXBatchDTO batchDTO : list){
            try {
                ...
                ylxOpenRequestService.prepareData(batchDTO);
            }catch (Exception e){
