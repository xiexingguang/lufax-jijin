...
@Component
public class YLXPrepareBuyConfirmDataJob extends BaseJob<YLXBatchDTO,Long> {
    @Autowired
    ...
    protected List<YLXBatchDTO> fetchList(int batchAmount) {
        return baseConfirmServie.getAfterTriggerDateBatchRecords(YLXBatchType.YLX_SLP_BUY_CONFIRM, asList(YLXBatchStatus2.RECEIVED_PREPARE), BatchRunStatus.IDLE);
    }
    ...
    protected void processList(List<YLXBatchDTO> list) {
        for(YLXBatchDTO batchDTO : list){
            try {
            	...
            	ylxBuyConfirmService.handleBuyConfirmData(batchDTO);
            }catch (Exception e){

                Logger.error(this, String.format("YLXPrepareBuyConfirmDataJob failed,batchId:%s", batchDTO.getId()),e);
            }finally {
