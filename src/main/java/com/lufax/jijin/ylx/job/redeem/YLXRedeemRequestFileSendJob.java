...
import com.lufax.jijin.ylx.request.service.YLXRedeemRequestSendFileService;
...
@Component
public class YLXRedeemRequestFileSendJob extends BaseJob<YLXBatchDTO,Long> {
    @Autowired
    private YLXRedeemRequestSendFileService ylxSendFileService;
    @Autowired
    private YLXBatchRepository ylxBatchRepository;
    @Override
    protected List<YLXBatchDTO> fetchList(int batchAmount) {
        return ylxSendFileService.getBatchSendFileRecords(YLXBatchType.YLX_SLP_REDEEM_REQUEST, YLXBatchStatus2.REQUEST_FILE_CREATED,BatchRunStatus.IDLE);
    }
    ...
    protected void processList(List<YLXBatchDTO> list) {
        for(YLXBatchDTO batchDTO : list){
            try {
                ...
                Logger.error(this, String.format("YLXRedeemRequestFileSendJob failed,batchId:%s", batchDTO.getId()),e);
