...
@Component
public class YLXRedeemRequestDataPrepareJob extends BaseJob<YLXBatchDTO,Long> {
    @Autowired
    ...
    protected List<YLXBatchDTO> fetchList(int batchAmount) {
        return ylxRedeemRequestService.getBatchRequestRecords(YLXBatchType.YLX_SLP_REDEEM_REQUEST, YLXBatchStatus2.REQUEST_INIT, BatchRunStatus.IDLE);
    }
    ...
    protected void processList(List<YLXBatchDTO> list) {
        for(YLXBatchDTO batchDTO : list){
            try {
                ...
                ylxRedeemRequestService.prepareData(batchDTO);
            }catch (Exception e){
                Logger.error(this,String.format("YLXRedeemRequestDataPrepareJob failed,batchId:%s",batchDTO.getId()),e);
            }finally {
