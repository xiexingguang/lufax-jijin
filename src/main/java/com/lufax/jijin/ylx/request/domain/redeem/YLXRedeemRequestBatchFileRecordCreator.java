...
@Component
public class YLXRedeemRequestBatchFileRecordCreator {
    @Autowired
    ...
    public void insertBatchFile(YLXBatchDTO batch){
        ...
        long count = ylxSellRequestDetailRepository.countSellRequestBybatchId(batch.getId());
        Logger.info(this, String.format("start to create redeem request file for %s,record count:%s",batch.getId(),count));
        if(count!=0){
            ...
            for(int i=1;i<fileNumLoop;i++){
                ...
                String fileName = "prod_redeem_ljs_op_"+target+"_"+num+".txt";
        ...
        Logger.info(this, String.format("end to create redeem request file for %s",batch.getId()));
    }
}
