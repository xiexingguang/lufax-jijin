...
@Component
public class YLXBuyRequestWriter implements IYLXRequestWriter{
	...
	public BigDecimal writeRequestContent(File newFile,YLXBatchFileDTO batchFile, long start, long end, long total,YLXBatchDTO batch,int fileSize){
        ...
        FileUtils.appendHeader(newFile,YlxConstants.BUY_FILE_TYPE, batchFile.getVersion(),batchFile.getOrgCode(), batchFile.getTrxDate(), total,amount);
        FileUtils.appendContent(newFile, BatchFileContentUtil.createColumnName(YLXBatchType.YLX_SLP_BUY_REQUEST));
        ...
        while (start <= end) {
            ...
            for (YLXBuyRequestDetailDTO dto : dtos) {
                content = BatchFileContentUtil.createBuyRequestContent(dto,batch);
