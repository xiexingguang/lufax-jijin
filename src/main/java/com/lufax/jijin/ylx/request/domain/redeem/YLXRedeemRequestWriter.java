...
@Component
public class YLXRedeemRequestWriter implements IYLXRequestWriter {
	...
	public BigDecimal writeRequestContent(File newFile,YLXBatchFileDTO batchFile, long start, long end, long total,YLXBatchDTO batch,int fileSize){
        ...
        //计算这个文件包含的记录集总金额
        BigDecimal amount = ylxSellRequestDetailRepository.sumAmountByBatchIdAndRownum(batch.getId(), start, end);

        FileUtils.appendHeader(newFile,YlxConstants.REDEEM_FILE_TYPE, batchFile.getVersion(),batchFile.getOrgCode(), batchFile.getTrxDate(), total,amount);
        FileUtils.appendContent(newFile, BatchFileContentUtil.createColumnName(YLXBatchType.YLX_SLP_REDEEM_REQUEST));
        ...
        while (start <= end) {
            ...
            List<YLXSellRequestDetailDTO> dtos = ylxSellRequestDetailRepository.getYLXSellRequestDTOsByBatchId(batch.getId(), start, targetEnd);
            String content = "";
            for (YLXSellRequestDetailDTO dto : dtos) {
                content = BatchFileContentUtil.createRedeemRequestContent(dto);
    ...
    public long getTotalRecords(long batchId) {
        ...
        return ylxSellRequestDetailRepository.countSellRequestBybatchId(batchId);
